...
public class RentalService {
    ...
    public Rental requestRental(Long memberNo, Long itemNo, LocalDate rentalStartDate, LocalDate rentalEndDate) {
        ...
        pointService.payoutForRental(item.getMember(), rental);
    ...
    public void cancelRental(Long rentalNo, Long memberNo) {
        ...
        pointService.refundForRental(rental.getMember(), rental);
        pointService.reverseForRental(rental.getItem().getMember(), rental);
    }
    ...
    @Transactional
    public void confirmReturn(Long rentalNo, Long providerMemberNo, ReturnStatus returnStatus, String returnMemo) {
        Rental rental = getRental(rentalNo);
        validateProvider(rental, providerMemberNo);
    ...
    public List<Rental> getRentalsAsRenter(Long memberNo) {
        return rentalRepository.findByMember(memberRepository.findById(memberNo)
                .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 회원입니다.")));
    }

    public List<Rental> getRentalsAsProvider(Long memberNo) {
        return rentalRepository.findByItem_Member(memberRepository.findById(memberNo)
                .orElseThrow(() -> new IllegalArgumentException("존재하지 않는 회원입니다.")));
    }
